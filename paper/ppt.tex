...
\begin{frame}{Loss Ordering Under Exposure Dominance}

\small

\textbf{Setup (common coupling):} Same Brownian motions, idiosyncratic factors, and LGD draws; only exposure schedules differ, with
\begin{equation*}
E^A_{\mathcal P_A}(t) \le E^A_{\mathcal P_B}(t)\quad \forall A,\; t\in[0,T].
\end{equation*}

\textbf{Loss ordering under exposure dominance:}
\begin{itemize}
\item Larger EAD implies larger contagion jumps: $\Delta Y^j(\mathcal P_B)\ge \Delta Y^j(\mathcal P_A)$.
\item SDE comparison gives ordered sector factors and obligor intensities.
\item Cox time-change gives ordered default times: $\tau^A_{\mathcal P_B}\le \tau^A_{\mathcal P_A}$.
\item Hence terminal losses are pathwise ordered:
\end{itemize}

\begin{equation*}
L_T^{\mathcal P_A} \le L_T^{\mathcal P_B}\quad \text{a.s.}
\end{equation*}

Therefore,
\begin{equation*}
L_T^{\mathcal P_A} \le_{\mathrm{st}} L_T^{\mathcal P_B}.
\end{equation*}

By monotonicity of Expected Shortfall,
\begin{equation*}
\mathrm{ES}_\alpha\!\left(L_T^{\mathcal P_A}\right) \le \mathrm{ES}_\alpha\!\left(L_T^{\mathcal P_B}\right),\quad \forall\alpha\in(0,1).
\end{equation*}

\textbf{Application:} $E_{\text{Linear}}\le E_{\text{French}}\le E_{\text{Bullet}}\le E_{\text{Neg.Am.}}$ \\
$\Rightarrow$ same ordering for $L_T$ and ES.
